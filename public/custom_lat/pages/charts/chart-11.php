...
<!-- Cabecera -->
	<div class="row mb-3">
		<div class="col-auto">
			...
			<div class="h6 mb-0"><span class="fw-bold">Periodo de Medición:</span> <?php echo $monthsYear[$monthToShow[60]] ?> a <?php echo $monthsYear[$periodopost] ?></div>
	...
	<table class="table align-middle table-bordered">
		<thead>
			<tr class="text-center">
				<th scope="col" class="c-mw-1">Incorporaciones Totales</th>
				<?php echo (intval($monthToShow[12]) <= intval($periodopost)) ? '<th scope="col">Ene</th>' : '<th scope="col" hidden>Ene</th>'; ?>
				...
				<?php echo (intval($monthToShow[9]) <= intval($periodopost)) ? '<th scope="col">Abr</th>' : '<th scope="col" hidden>Abr</th>'; ?>
				...
				<?php echo (intval($monthToShow[5]) <= intval($periodopost)) ? '<th scope="col">Ago</th>' : '<th scope="col" hidden>Ago</th>'; ?>
				...
				<?php echo (intval($monthToShow[1]) <= intval($periodopost)) ? '<th scope="col">Dic</th>' : '<th scope="col" hidden>Dic</th>'; ?>
...
<div class="table-responsive mt-4">
	<table class="table align-middle table-bordered">
		<thead>
			<tr class="text-center">
				<th scope="col" class="c-mw-1">Activos Mensuales (ABIs + CPs)</th>
				<?php echo (intval($monthToShow[12]) <= intval($periodopost)) ? '<th scope="col">Ene</th>' : '<th scope="col" hidden>Ene</th>'; ?>
				<?php echo (intval($monthToShow[11]) <= intval($periodopost)) ? '<th scope="col">Feb</th>' : '<th scope="col" hidden>Feb</th>'; ?>
				<?php echo (intval($monthToShow[10]) <= intval($periodopost)) ? '<th scope="col">Mar</th>' : '<th scope="col" hidden>Mar</th>'; ?>
				<?php echo (intval($monthToShow[9]) <= intval($periodopost)) ? '<th scope="col">Abr</th>' : '<th scope="col" hidden>Abr</th>'; ?>
		    ...
		    options: {
				...
				scales: {
			      	y: {
			        	...
			        	title: {
			          		display: true,
			          		text: 'No. Incorporaciones Totales'
			        	},
						beginAtZero: true
			      	}
			    }
	...
	//Gráfica Activos
		...
		var chart11Graph2Detail = new Chart(chart11Graph2, {
		    ...
		    data: {
		        ...
		        datasets: [
			        {
			            label: 'Activos Mensuales (ABIs + CPs)',
		    ...
		    options: {
				...
				plugins: {
					title: {
						display: true,
						text: 'Activos Mensuales (ABIs + CPs)'
					},
				},
				scales: {
			      	y: {
			        	...
			        	title: {
			          		display: true,
			          		text: 'No. Activos Mensuales (ABIs + CPs)'
			        	},
						beginAtZero: true
			      	}
			    }
